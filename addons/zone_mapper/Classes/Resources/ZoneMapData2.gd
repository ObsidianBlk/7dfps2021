...
export var minimum_height : float = 2.0
...
func add_zone(f : float, c : float) -> void:
	if c > f and c - f >= minimum_height:
		var zone = {
			"floor": f,
			"ceiling": c,
			"rect":Rect2(),
...
func set_zone_floor(zidx : int, flr : int, adj_ceiling : bool = false) -> void:
	if zidx >= 0 and zidx < _Map.size():
		if _Map[zidx].ceiling > flr:
			var nheight = _Map[zidx].ceiling - flr
			if nheight >= minimum_height or adj_ceiling:
				_Map[zidx].floor = flr
			if nheight < minimum_height and adj_ceiling:
				pass

func set_zone_ceiling(zidx : int, ceiling : int) -> void:
	if zidx >= 0 and zidx < _Map.size():
		if _Map[zidx].floor < ceiling:
			_Map[zidx].ceiling = ceiling

func set_zone_dimensions(zidx : int, flr : int, ceiling : int) -> void:
	pass
